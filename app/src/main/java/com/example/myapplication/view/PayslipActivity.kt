...
class PayslipActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Support both MONTH/YEAR and FILTER_MONTH/FILTER_YEAR for robustness
        val filterMonth = intent.getIntExtra("MONTH", intent.getIntExtra("FILTER_MONTH", -1))
        val filterYear = intent.getIntExtra("YEAR", intent.getIntExtra("FILTER_YEAR", -1))
        ...
        tvEmployeeName.text = getString(R.string.employee_label, MyApplication.sessionManager.fetchUserName() ?: getString(R.string.not_available))
        tvEmployeeId.text = getString(R.string.employee_id_label, MyApplication.sessionManager.fetchEmpIdSecondary() ?: getString(R.string.not_available))
        ...
        if (!empSalaryId.isNullOrEmpty()) {
            payrollViewModel.fetchPayrollDetail(empSalaryId, filterMonth, filterYear)
        } else {
        ...
        btnDownload.setOnClickListener {
            if (filterMonth != -1 && filterYear != -1) {
                payrollViewModel.downloadPayslipByMonth(filterMonth, filterYear)
            } else if (!empSalaryId.isNullOrEmpty()) {
        ...
        payrollViewModel.payrollData.observe(this) { data ->
            ...
            data.let {
                ...
                if (!it.empId.isNullOrEmpty()) {
                    tvEmpId.text = getString(R.string.employee_id_label, it.empId)
                }
    ...
    private fun addComponentRow(container: LinearLayout, component: SalaryComponent, isDeduction: Boolean) {
        ...
        val amountTv = TextView(context).apply {
            text = String.format(Locale.getDefault(), getString(R.string.currency_format), component.amount ?: 0.0)
