    ...
    private fun handleSuccessfulDualLogin(result: DualLoginResult) {
        try {
            ...
            result.emsToken?.let { token ->
                ...
                // Try response body first, then token
                val emsEmpIdFromResponse = result.emsResponse?.user?.employeeId
                val (_, emsEmpIdFromToken, _) = dualLoginRepository.decodeToken(token)
                val finalEmsEmpId = emsEmpIdFromResponse ?: emsEmpIdFromToken

                finalEmsEmpId?.let { id ->
                    MyApplication.sessionManager.saveEmpIdEms(id)
                    Log.d("LoginViewModel", "✅ EMS empId saved: $id")
                } ?: run {
                    Log.e("LoginViewModel", "❌ EMS empId is missing from both response and token")
                }
            }

            // Save user info if available (Try PMS then EMS)
            val userName = result.pmsResponse?.user?.userName 
                ?: result.emsResponse?.user?.userName 
                ?: result.emsResponse?.email
    ...
    private fun handlePartialLogin(result: DualLoginResult) {
        try {
            ...
            result.emsToken?.let { token ->
                ...
                } ?: Log.e("LoginViewModel", "❌ EMS empId missing from both response and token (partial login)")
            }
