...
class CombinedRepository(
    ...
    suspend fun fetchPmsLeaveBalance(empId: String): Result<LeaveBalanceResponse> {
        return try {
            ...
            if (response.isSuccessful) {
                response.body()?.let { Result.success<LeaveBalanceResponse>(it) }
                    ?: Result.failure<LeaveBalanceResponse>(Exception("PMS Leave balance empty response"))
            } else {
                Result.failure<LeaveBalanceResponse>(Exception("PMS Leave balance failed: ${response.code()}"))
            }
        } catch (e: Exception) {
    ...
    suspend fun fetchPmsPayrollDetails(empId: String): Result<PayrollDetailsResponse> {
        return try {
            ...
            if (response.isSuccessful) {
                response.body()?.let { Result.success<PayrollDetailsResponse>(it) }
                    ?: Result.failure<PayrollDetailsResponse>(Exception("PMS Payroll details empty response"))
            } else {
                Result.failure<PayrollDetailsResponse>(Exception("PMS Payroll details failed: ${response.code()}"))
            }
        } catch (e: Exception) {
        ...
        return try {
            ...
            if (response.isSuccessful) {
                ...
                response.body()?.let { body ->
                    ...
                    Result.success<PayrollDetailsResponse>(body)
                } ?: Result.failure<PayrollDetailsResponse>(Exception("Empty response body from EMS server"))
            } else {
    ...
    private suspend fun fetchPayrollFromPms(
        ...
        return try {
            ...
            if (response.isSuccessful) {
                ...
                response.body()?.let { Result.success<PayrollDetailsResponse>(it) }
                    ?: Result.failure<PayrollDetailsResponse>(Exception("Empty response body from PMS server"))
            } else {
                val errorBody = response.errorBody()?.string() ?: "Unknown error"
    ...
    suspend fun fetchPayrollStructures(): Result<List<PayStructure>> {
        return try {
