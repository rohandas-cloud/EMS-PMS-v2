...
class LoginRepository(
    ...
    suspend fun logout(): Boolean {
        return try {
            ...
            Log.e("LoginRepository", "Logout error: ${e.message}")
            false
        }
    }
    ...
    companion object {
        ...
        fun decodeToken(token: String): Triple<String?, String?, String?> {
            return try {
                ...
                val userId = getStringOrNull("userId") ?: getStringOrNull("id") ?: getStringOrNull("uid") ?: getStringOrNull("sub")
                val empId = getStringOrNull("empId") ?: getStringOrNull("employeeId") ?: getStringOrNull("emp_id") ?: getStringOrNull("custom:employeeId") ?: userId
                
                // If sub was used for userId, it's likely not the email. Check 'email' claim specifically.
                val email = getStringOrNull("email") ?: getStringOrNull("username") ?: getStringOrNull("sub")

                Log.d("LoginRepo", "Decoded Token -> userId: $userId, empId: $empId, email: $email")
